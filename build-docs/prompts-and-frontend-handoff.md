...
1. **Join** — nickname + "Your native language / mother tongue" + "Other languages you speak" (chips, multi-select from a fixed list: Assamese, Bengali, Hindi, English, Kannada, Tamil, Telugu, Malayalam, Marathi, Odia, Nepali, Bhojpuri, + free-text "other"). Native language is excluded from the common-language chips automatically. Show a dual-script summary of selections before join. Big friendly copy: "Speak your language. English has had enough turns."
2. **Queued** — "Finding you a partner…" + the player's mother tongue / known-language labels + live leaderboard below (kills perceived wait). Backend matchmaking uses speakable = native ∪ common; when English is shared, `common_lang` prefers `en` so labels stay English for the demo.
